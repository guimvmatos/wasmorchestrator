...
fn main() -> std::io::Result<()> {
    ...
    loop {
        ...
        if last_table_update.elapsed() >= Duration::from_secs(15) {
            ...
            let (new_table, scores) = decide_routing(&nodes_health);

            if !new_table.is_empty() {
                routing_table.table = new_table;
                routing_table.node_scores = scores;
                println!("Nova configuração decidida: {:?}", routing_table.table);

                // Seu código de envio original abaixo...
                if let Ok(mut stream) = TcpStream::connect("127.0.0.1:9999") {
                    ...
                    println!("Tabela enviada com sucesso.");
                }
            }
