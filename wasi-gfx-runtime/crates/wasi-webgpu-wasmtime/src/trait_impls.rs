...
use crate::wasi_gfx_runtime::wasi_webgpu::sync_io;
use core::slice;
...
impl<'a> sync_io::Host for WasiWebGpuCtx<'a> {
    fn map_and_read_sync(
        &mut self,
        buffer: Resource<webgpu::GpuBuffer>,
        ...
        let host_map = if mode == webgpu::GpuMapMode::READ {
            wgpu_core::device::HostMap::Read
        } else if mode == webgpu::GpuMapMode::WRITE {
            wgpu_core::device::HostMap::Write
        } else {
            return Ok(Err("modo de mapeamento inválido".to_string()));
        };

        // Callback do wgpu avisa via channel quando o mapeamento termina.
        // A thread de background (poll_all_devices em loop, em main.rs)
        // é quem de fato faz esse callback disparar.
        ...
        // Bloqueia a THREAD NATIVA do host aqui (não a task do wasm — essa
        // função é uma func comum, sem "async" na WIT). Isso é permitido
        // pelo Canonical ABI porque, do ponto de vista do guest, é só uma
        // chamada de import comum que demora antes de retornar.
        let map_result = match rx.recv_timeout(std::time::Duration::from_secs(30)) {
            Ok(result) => result,
            Err(_) => return Ok(Err("timeout de 30s esperando map_async".to_string())),
        };

        if let Err(e) = map_result {
            return Ok(Err(format!("mapeamento falhou no host: {e:?}")));
        }

        let (ptr, len) = self.instance.buffer_get_mapped_range(buffer_id, offset, size)?;
        let data = unsafe { std::slice::from_raw_parts(ptr.as_ptr(), len as usize) }.to_vec();

        self.instance.buffer_unmap(buffer_id)?;
