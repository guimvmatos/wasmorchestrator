...
// shader, mesmo bind group layout). Recriar isso a cada camada é o que
// estava dominando o exec_ms observado no pipeline (400-600ms por camada).
struct GpuContext {
...
fn build_gpu_context() -> GpuContext {
    ...
    // Prova direta de qual GPU está sendo usada — só imprime uma vez agora,
    // na primeira chamada, já que o contexto é reaproveitado depois.
