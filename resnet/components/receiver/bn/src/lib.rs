...
fn handle_client(mut stream: TcpStream, initialized: &mut i32) -> std::io::Result<()> {
    ...
    loop {
        ...
        // 2. Aloca o buffer de saída dinamicamente
        let total_size = (p.channels * p.h_in * p.w_in) as usize;
        input_img.output = vec![0.0f32; total_size];

        let scale = if !p.scale_path.is_empty() { load_bin(&p.scale_path) } else { Vec::new() };
        let beta  = if !p.beta_path.is_empty()  { load_bin(&p.beta_path) } else { Vec::new() };
        let mean  = if !p.mean_path.is_empty()  { load_bin(&p.mean_path) } else { Vec::new() };
        let var   = if !p.var_path.is_empty()   { load_bin(&p.var_path) } else { Vec::new() };
...
impl bindings::exports::wasi::cli::run::Guest for Component {
    fn run() -> Result<(), ()> {
        ...
        let port = args.get(1).map(|s| s.as_str()).unwrap_or("8082"); //fallback.. se nao passar por argumento ele vai coloar essa
