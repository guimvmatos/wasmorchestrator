...
type WitData = bindings::planner::reluworld::plan::Data;
...
fn handle_client(mut stream: TcpStream, initialized: &mut i32) -> std::io::Result<()> {
    ...
    let MY_ID: u8 = 3; //#### TODO colocar o numero da funcao aqui...
    ...
    loop {
       ...
       // 1. CARREGA O GRAFO E OBTÉM O STEP ATUAL
        ...
        // 2. Aloca buffer de saída e executa o kernel
        let total_size = input_img.input.len();
        input_img.output = vec![0.0f32; total_size];

        let mut status = bindings::planner::reluworld::plan::relu(&input_img);
...
impl bindings::exports::wasi::cli::run::Guest for Component {
    fn run() -> Result<(), ()> {
        ...
        let port = args.get(1).map(|s| s.as_str()).unwrap_or("8083"); //fallback.. se nao passar por argumento ele vai coloar essa
