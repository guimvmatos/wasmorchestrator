...
/*
// Buffer estático para guardar o 1º ramo do Add até o 2º chegar
static ADD_BRANCH_BUFFER: Mutex<Option<HashMap<(u32, u32), Vec<f32>>>> = Mutex::new(None);

fn get_or_init_branch_buffer() -> &'static Mutex<Option<HashMap<(u32, u32), Vec<f32>>>> {
    let mut lock = ADD_BRANCH_BUFFER.lock().unwrap();
    if lock.is_none() {
...
fn handle_client(mut stream: TcpStream, initialized: &mut i32) -> std::io::Result<()> {
    ...
    let MY_ID: u8 = 0; //#### TODO colocar o numero da funcao aqui...
...
let graph: ModelGraph = serde_json::from_reader(graph_file).expect("Erro no parser do resnet18DFG.json");
        ...
        while let Some((current_step, current_input)) = queue.pop_front() {
            ...
            match current_node.kernel_type {
                ...
                _ => panic!("Kernel type {} não suportado", current_node.kernel_type),
            }
