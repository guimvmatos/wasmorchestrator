...
type WitData = bindings::planner::gapworld::plan::Data;
...
fn handle_client(mut stream: TcpStream, initialized: &mut i32) -> std::io::Result<()> {
    ...
    let MY_ID: u8 = 6; //#### TODO colocar o numero da funcao aqui...
...
let graph: ModelGraph = serde_json::from_reader(graph_file).expect("Erro no parser do resnet18DFG.json");
        ...
        input_img.output = vec![0.0f32; 512];

        // 3. Executa o kernel C do GAP diretamente
        let mut status = bindings::planner::gapworld::plan::gap(
            &input_img.input,
            &input_img,
        );
...
impl bindings::exports::wasi::cli::run::Guest for Component {
    fn run() -> Result<(), ()> {
        ...
        let port = args.get(1).map(|s| s.as_str()).unwrap_or("8086"); //fallback.. se nao passar por argumento ele vai coloar essa
