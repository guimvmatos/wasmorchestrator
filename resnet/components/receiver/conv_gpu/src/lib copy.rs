/*
TODO LIST
1 - PARAMETROS DE PESOS E CONVOLUÇÕES: Estou definindo manualmente. Preciso que ele leia o arquivo .json e retire de lá os pesos
2 - 
...
fn handle_client(mut stream: TcpStream, initialized: &mut i32) -> std::io::Result<()> {
    ...
    loop {
        ...
        // Parâmetros e pesos da convolução
        let c_out: u32 = 64;
        let c_in: u32 = 3;
        let h_in: u32 = 224;
        let w_in: u32 = 224;
        ...
        let stride: u32 = 2;
        let padding: u32 = 3;
        //let weights: Vec<f32> = vec![0.1f32; (c_out * c_in * kh * kw) as usize];

        // Calcula dimensões de saída e ALOCA o vetor de output
        let out_h = ((h_in + 2 * padding - kh) / stride) + 1; // 112
        let out_w = ((w_in + 2 * padding - kw) / stride) + 1; // 112
        let output_size = (c_out * out_h * out_w) as usize;   // 64 * 112 * 112 = 802.816 floats
        input_img.output = vec![0.0f32; output_size];

        let weight_path = "../bin_weights/resnetv15_conv0_weight.bin";
        let weights_bytes = std::fs::read(weight_path)
            .expect("Erro ao carregar o arquivo binário de pesos");

        // Conversão ultra-rápida de &[u8] para Vec<f32> em tempo zero
        let weights: Vec<f32> = unsafe {
        ...
        //let mut status = bindings::planner::convworld::plan::conv(&input_img);
