...
fn spawn_wasm_worker(func_id: u8, port: u16, node_ip: &str) -> Child {
    ...
    Command::new("wasmtime")
        ...
        .args(["--wasi", "inherit-network", "--dir", ".", "--dir", "../bin_weights/"])
        .arg(&wasm_file)
        .arg(port.to_string())
        .arg(node_ip)
        .spawn()
...
fn send_to_orchestrator(metrics: SystemMetrics) {
    let addr = "0.0.0.0:9998"; // #### TODO: Porta onde o Orchestrador estará ouvindo telemetria. Colocar o Ip do nó onde esta o orquestrador
...
fn sync_workers(routing_data: &RoutingTable, active_workers: &mut HashMap<u8, Child>, node_id: u8, node_ip: &str) -> Vec<FunctionInfo> {
    ...
    if let Some(my_tasks) = routing_data.assignments.get(&node_id) {
        for &func_id in my_tasks {
            if !active_workers.contains_key(&func_id) {
                //let port = match func_id { 1 => 8081, 2 => 8082, 3 => 8083,4 => 8084, _ => 8080 };
                let port: u16 = 8080 + func_id as u16; // 1 -> 8081, 2 -> 8082, 3 -> 8083, 4 -> 8084, ...
    ...
    for &id in active_workers.keys() {
        //let port = match id { 1 => 8081, 2 => 8082, 3 => 8083, 4 => 8084, _ => 8080 };
        let port: u16 = 8080 + id as u16;
...
fn main() -> std::io::Result<()> {
    let listener = TcpListener::bind("0.0.0.0:9999")?; //#### TODO: colocar o ip da maquina local ou 0.0.0.0. é por onde o seu sys vai ouvir
    let my_node_id = 1; //#### TODO: Trocar o numero do nó
    let my_ip = "127.0.0.1"; //#### TODO: Colocar o numero do ip da maquina onde esta nó esta.
